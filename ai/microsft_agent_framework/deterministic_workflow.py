...
class EmailPreprocessor(Executor):
    ...
    async def handle_email(
        self, email: EmailRequest, ctx: WorkflowContext[EmailContent]
    ) -> None:
...
class LegitimateMessageHandler(Executor):
    ...
    async def handle_spam_detection(
        ...
        if spam_result.is_spam:
            raise RuntimeError(
                "Message is spam, cannot respond with message responder."
            )
    ...
    async def handle_processing_result(
        ...
        if result.is_spam:
            ...
            if result.was_human_reviewed:
                ...
                human_decision = (
                    result.human_override if result.human_override else "unknown"
                )
...
workflow = (
    ...
    .add_switch_case_edge_group(
        ...
        [
            Case(
                condition=lambda x: isinstance(x, SpamDetectorResponse) and x.is_spam,
                target=spam_handler,
            ),
            Default(
